import {
  ...
  export const getGenres = createAsyncThunk("ubinge/genres",async()=>{
    const {
        data:{genres},
        } = await axios.get(
  ...
  export const fetchMovies= createAsyncThunk("ubinge/trending",async({type},thunkApi)=>{
...
//   return getRawData(`${TMDB_BASE_URL}/discover/${type}?api_key${API_KEY}&with_genres=${genres}`)
  ...
  export const store = configureStore({
    reducer: {
      ubinge: UbingeSlice.reducer,
    },
  });
  
  export const { setGenres, setMovies } = UbingeSlice.actions;
